...
| File: wave.c
|
| Description: Wave library routines.
...
|
| Description: This routine has to be called before WaveReadFile as it 
|   searchs for the chunk to descend into for reading, that is, the 'data' 
|   chunk.  For simplicity, this used to be in the open routine, but was
|   taken out and moved to a separate routine so there was more control 
|   on the chunks that are before the data chunk, such as 'fact', etc.
|___________________________________________________________________*/
...
/*____________________________________________________________________
|
| Function: WaveCreateFile
|
| Description:
|   This routine will create a wave file for writing.  This will 
|   automatically overwrite any existing file with the same name, so be 
|   careful and check beforehand!!!
|
...
/*____________________________________________________________________
|
| Function: WaveStartDataWrite
|
| Description: This routine has to be called before any data is written 
|   to the wave output file, via wavewritefile.  This sets up the data 
|   to write, and creates the data chunk.
|___________________________________________________________________*/
...
/*____________________________________________________________________
|
| Function: WaveWriteFile
|
| Description:  This routine will write out data to a wave file. Returns 
...
/*____________________________________________________________________
|
| Function: WaveCloseWriteFile
|
| Description: This routine will close a wave file used for writing.  
...
/*____________________________________________________________________
|
| Function: WaveCopyUselessChunks
|
| Description: This routine will copy from a source wave file to a 
|   destination wave file all those useless chunks (well, the ones useless 
|   to conversions, etc --> apparently people use them!).  The source 
|   will be seeked to the begining, but the destination has to be at a 
|   current pointer to put the new chunks.  This will also seek back to 
|   the start of the wave riff header at the end of the routine.
|
|   Returns 0 if successful, else the error code.  If this routine fails, 
|   it still attemps to seek back to the start of the wave riff header, 
|   though this too could be unsuccessful.
|
...
|
| Description: This routine loads a full wave file into memory.  Be
|   careful, wave files can get pretty big these days :). 
|
|   Returns 0 if successful, else the error code.
|
...
/*____________________________________________________________________
|
| Function: WaveSaveFile
|
| Description: This routine saves a wave file in currently in memory.
...
| Function: WaveSaveFile
|
| Description: This function will open a wave input file and prepare it 
|   for reading, so the data can be easily read with WaveReadFile. 
|
...
/*____________________________________________________________________
|
| Function: Wave2ReadFile
|
| Description: This will read wave data from the wave file.  Make sure 
|   we're descended into the data chunk, else this will fail bigtime!
|
|     hmmioIn       - Handle to mmio.
|     cbRead        - # of bytes to read.   
|     pbDest        - Destination buffer to put bytes.
...
| Function: Wave2ReadFile
|
| Description: This will close the wave file opened with WaveOpenFile.  
|
|   Returns 0 if successful, non-zero if there was a warning.
|
|     phmmioIn - Pointer to the handle to input MMIO.
