...
*
*		Contains fixes (by TR) that allow this header to compile properly with VC8.
*
*-------------------------------------------------------------------------------
...
//            ULONG cbNeeded = (wcslen(pSrc) + 1) * sizeof(WCHAR);
            ULONG cbNeeded = (ULONG)((wcslen(pSrc) + 1) * sizeof(WCHAR));
...
//        return (m_psz == NULL)? 0 : wcslen(m_psz);
        return (unsigned int)((m_psz == NULL)? 0 : wcslen(m_psz));
    }
...
//            ULONG lenSrc = wcslen(pszSrc);
            ULONG lenSrc = (ULONG) wcslen(pszSrc);
            if (lenSrc)
            {
                ULONG lenMe = Length();
...
//        ULONG lenSrc2 = pszSrc2 ? wcslen(pszSrc2) : 0;
				ULONG lenSrc1 = (ULONG)(pszSrc1 ? wcslen(pszSrc1) : 0);
        ULONG lenSrc2 = (ULONG)(pszSrc2 ? wcslen(pszSrc2) : 0);
...
//    ULONG ulLen = pszOptAttribs ? wcslen(pszOptAttribs) + ulLenVendorPreferred + 1 : ulLenVendorPreferred;
    ULONG ulLen = (ULONG)(pszOptAttribs ? wcslen(pszOptAttribs) + ulLenVendorPreferred + 1 : ulLenVendorPreferred);
...
//        cTotalChars += wcslen(ppWords[i])+1;
        cTotalChars += (ULONG)(wcslen(ppWords[i])+1);
    }
